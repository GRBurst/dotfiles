    ...
    if updates > updates_gap then
		for i in pairs(graph_settings) do
			...
			if (graph_settings[i].draw_me==nil or conky_parse(tostring(graph_settings[i].draw_me)) == "1") then 
 		    ...
 		    if conky_parse("${if_existing /proc/net/route enp6s0}1${else}0${endif}") then 
				graph_settings[i].arg = "enp6s0"
				draw_graph(graph_settings[i])
		    elseif conky_parse("${if_existing /proc/net/route wlp0s26u1u4u4}1${else}0${endif}") then 
				graph_settings[i].arg = "wlp0s26u1u4u4"
				draw_graph(graph_settings[i])
    		    else 
